...
def update(frame):
    ...
    u, v, w = scale_vectors(u, v, w)
    # uE, vE, wE = scale_vectors(uE, vE, wE, 0.5, 8e5)
    # uB, vB, wB = scale_vectors(uB, vB, wB, 0.5, 8e5)

    quiverA.remove()
    # quiverE.remove()
    # quiverB.remove()
    quiverA = ax.quiver(df_A.x, df_A.y, df_A.z, u, v, w, length=1, normalize=False, color='blue')
    # quiverE = ax.quiver(df_E.x, df_E.y, df_E.z, uE, vE, wE, length=8e5, normalize=False, color='red')
    # quiverB = ax.quiver(df_B.x, df_B.y, df_B.z, uB, vB, wB, length=8e5, normalize=False, color=(0.9, 0.9, 0.0))
...
ts = time.time()
fN = "../../graphs/A/A_example_" + datetime.datetime.fromtimestamp(ts).strftime('%Y-%m-%d-%H-%M-%S') + ".gif"

# ani.save(filename=fN, writer="imagemagick", dpi=300)
plt.show()
